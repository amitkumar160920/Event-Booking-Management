...
#trigger
  ...
  $stmt = $conn->prepare("INSERT INTO Customer_reg (name, email, mobile1, mobile2, address, start_date, end_date, start_time, comment, shop_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssssssssi", $name, $email, $contactNo1, $contactNo2, $venue, $sd, $ed, $stime, $comment, $shop_id);
  ...
  //                    $row = mysqli_fetch_assoc($result);
                     ...
                     if ($stmt->execute()) {
                      myAlert("Successfully Registered");
                      $sql = "SELECT S.email, S.mobile FROM sreg S WHERE id = ?";
